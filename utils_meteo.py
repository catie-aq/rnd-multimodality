...
def plot_model_comparison(dataframe: pd.DataFrame, target: str) -> None:
    """
    ...
    plt.title("Comparison between TiDE and Chronos On Humidity data")
...
def convert_forecast_to_pandas(
    ...
    forecast_pd = holdout_set[["number_sta", "date"]]
...
def plot_actuals_forecast(
    actuals_data: pd.DataFrame, forecast_data: pd.DataFrame, title: str, target: str
) -> None:
    """
...
def transform_predictions_to_pandas(predictions: list, target: str, pred_list: list, quantiles: list) -> pd.DataFrame:
    ...
    for p, pdf in zip(predictions, pred_list):
        ...
        # add unique id
        temp["unique_id"] = list(pdf.static_covariates_values())[0][0]
